...
public class OP : Singleton<OP>
{
    ...
    public static void ClearCachePool()
    {
        ...
        OP.m_GoTag.Clear();
        OP.m_IDCnt.Clear();
        while (OP.Ins.transform.childCount > 0)
        {
            UnityEngine.Object.DestroyImmediate(OP.Ins.transform.GetChild(0).gameObject);
        }
    }
    ...
    public static void ReturnCacheGameObject(GameObject go)
    {
        if (OP.CachePanel == null)
        {
            ...
            OP.CachePanel.transform.SetParent(OP.Ins.transform);
        }
        if (go == null)
        {
            return;
...
public static class GOExt
{
    ...
    {
        Transform p = parent == null ? OP.Ins.transform : parent;
